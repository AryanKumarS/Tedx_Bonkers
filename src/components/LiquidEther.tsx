    ...
    class MouseClass {
      ...
      onDocumentTouchStart(event: TouchEvent) {
        if (event.touches.length === 1) {
          ...
          // Inject a small impulse so stationary touches register
          this.coords_old.copy(this.coords).addScalar(0.02); 
...
`;
    ...
    class Advection extends ShaderPass {
      ...
      update(params?: { dt?: number; isBounce?: boolean; BFECC?: boolean; dissipation?: number }) {
        const { dt, isBounce, BFECC, dissipation } = params || {};
        if (!this.uniforms) return;
        if (typeof dt === "number") this.uniforms.dt.value = dt;
        if (typeof dissipation === "number") this.uniforms.dissipation.value = dissipation;
